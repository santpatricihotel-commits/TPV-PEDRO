...
const productos = [
  { id: 1, nombre: 'SEMI 1/8', precio: 6.50, color: 'bg-yellow-500' },
  { id: 2, nombre: 'CURAT 1/8', precio: 7.50, color: 'bg-amber-500' },
  { id: 3, nombre: 'AÑEJO 1/8', precio: 6.00, color: 'bg-orange-500' },
  { id: 4, nombre: 'ES MOLL', precio: 18.00, color: 'bg-rose-500' },
  { id: 5, nombre: 'SA VERMELLA', precio: 22.00, color: 'bg-red-600' },
  { id: 6, nombre: 'ES RUPIT', precio: 19.00, color: 'bg-stone-400' },
  { id: 7, nombre: 'ES RUPIT', precio: 19.00, color: 'bg-stone-800' },
];
...
function App() {
  ...
  // 💶 Cálculo del cambio
  const recibidoNum = parseFloat(String(dineroRecibido).replace(',', '.')) || 0;
  const cambio = recibidoNum - total;
  const recibidoSuficiente = recibidoNum >= total - 0.001;

  const sumarBillete = (valor) => {
    const nuevo = (parseFloat(String(dineroRecibido).replace(',', '.')) || 0) + valor;
    setDineroRecibido(nuevo.toFixed(2));
  };
      ...
      mostrarPago && React.createElement('div', { className: "fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50" },
        React.createElement('div', { className: "bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden" },
          estadoPago === 'idle' && React.createElement(React.Fragment, null,
            ...
            React.createElement('div', { className: "p-6" },
              ...
              React.createElement('div', { className: "space-y-4" },
                ...
                React.createElement('button', {
                  onClick: () => { setDineroRecibido(''); setEstadoPago('efectivo'); },
          ...
          estadoPago === 'efectivo' && React.createElement(React.Fragment, null,
            ...
            React.createElement('div', { className: "p-6" },
              React.createElement('div', { className: "text-center mb-4" },
                React.createElement('p', { className: "text-gray-500" }, "Total a cobrar"),
                React.createElement('p', { className: "text-3xl font-bold text-indigo-600" }, formatoPrecio(total))
              ),

              React.createElement('label', { className: "block text-gray-600 font-semibold mb-2" }, "¿Cuánto dinero te han dado?"),
              ...
              React.createElement('div', { className: "grid grid-cols-3 gap-2 mb-4" },
                React.createElement('button', {
                  ...
                  className: "bg-indigo-100 text-indigo-700 py-3 rounded-lg font-bold hover:bg-indigo-200"
                }, "Exacto"),
                [5, 10, 20, 50].map(v =>
                  React.createElement('button', {
                    key: v,
                    onClick: () => sumarBillete(v),
                    className: "bg-gray-100 text-gray-700 py-3 rounded-lg font-bold hover:bg-gray-200"
                  }, "+" + v + "€")
                ),
                React.createElement('button', {
                  onClick: () => setDineroRecibido(''),
                  className: "bg-red-100 text-red-600 py-3 rounded-lg font-bold hover:bg-red-200"
                }, "Borrar")
              ),
              ...
              React.createElement('div', {
                className: "rounded-xl p-4 text-center mb-4 " +
                  (recibidoNum === 0 ? "bg-gray-100" : recibidoSuficiente ? "bg-green-100" : "bg-red-100")
              },
                React.createElement('p', { className: "text-gray-600 font-semibold" },
                  recibidoNum === 0 ? "Introduce el importe recibido" :
                  recibidoSuficiente ? "Cambio a devolver" : "Falta dinero"
                ),
                React.createElement('p', {
                  className: "text-4xl font-bold " +
                    (recibidoNum === 0 ? "text-gray-400" : recibidoSuficiente ? "text-green-600" : "text-red-500")
                },
                  recibidoNum === 0 ? "—" :
                  recibidoSuficiente ? formatoPrecio(Math.max(0, cambio)) : formatoPrecio(Math.abs(cambio))
                )
              ),
          ...
          estadoPago === 'success' && React.createElement('div', { className: "p-12 text-center" },
            ...
            // Cambio en pantalla de éxito (solo efectivo)
            metodoPago === 'efectivo' && recibidoNum > 0 && React.createElement('div', { className: "mt-6 bg-amber-100 rounded-xl p-4" },
              React.createElement('p', { className: "text-amber-700 font-semibold" }, "💶 Recibido: " + formatoPrecio(recibidoNum)),
              React.createElement('p', { className: "text-amber-800 font-bold text-3xl mt-1" }, "Cambio: " + formatoPrecio(Math.max(0, cambio)))
            ),
            metodoPago === 'efectivo' && React.createElement('button', {
              onClick: finalizarVenta,
              className: "mt-6 w-full bg-indigo-600 text-white py-4 rounded-xl text-xl font-bold hover:bg-indigo-700"
            }, "Nueva venta")
          ),
